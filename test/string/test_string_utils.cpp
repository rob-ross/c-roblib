...
TEST(CharRingBuffer10, TestAdd) {
    ...
    sutil_print_repr_CharRingBuffer10(&crb);

    sutil_add_to_buffer_CharRingBuffer10(&crb, 10, "abcdefghij");
    sutil_print_repr_CharRingBuffer10(&crb);
    ...
    sutil_add_to_buffer_CharRingBuffer10(&crb, 5, "abcde");
    EXPECT_EQ(crb.length, 5);
    EXPECT_EQ(crb.end_index, 5);
    sutil_print_repr_CharRingBuffer10(&crb);

    sutil_add_to_buffer_CharRingBuffer10(&crb, 10, "fghijklmno");
...
TEST(CharRingBuffer10, TestRead) {
    ...
    // read 4 values
    for (int i = 0; i < 4; ++i) {
        sutil_get_next_char_CharRingBuffer10(&crb);
    }
    ...
    EXPECT_EQ(sutil_get_next_char_CharRingBuffer10(&crb), EOF);
...
TEST(CharRingBuffer10, TestAddReadAddRead) {
    ...
    sutil_add_to_buffer_CharRingBuffer10(&crb, 12, "123456789XYZ");
    EXPECT_EQ(crb.length, 10);
    EXPECT_EQ(crb.start_index, 0);
    EXPECT_EQ(crb.end_index, 0);
    sutil_print_repr_CharRingBuffer10(&crb);
