 ...
 */
    ...
    // Our StackAllocator struct would build on Arena:
    // we need the exact same data for our meta stack as the Arena and payload stack.

    // Like with arena_allocator, we store the Arena struct on the first block after the BlockHeader
